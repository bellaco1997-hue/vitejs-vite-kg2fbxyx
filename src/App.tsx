...
// Build 8-week progress data for a given group+exercise
function buildProgressData(log: any, groupId: string, exId: number) {
  ...
  // Find all sessions for this group+exercise, grouped by week
  const sessionsByWeek: { [weekIdx: number]: { date: string; sets: any[] }[] } = {};
  Object.entries(log || {}).forEach(([key, val]: any) => {
    ...
    const wIdx = weeks.findIndex(w => dateKey >= w.start && dateKey <= w.end);
    if (wIdx === -1) return;
    if (!sessionsByWeek[wIdx]) sessionsByWeek[wIdx] = [];
    sessionsByWeek[wIdx].push({ date: dateKey, sets });
  });

  // How many sets max across all sessions
  let maxSets = 0;
  Object.values(sessionsByWeek).forEach(sessions => {
    sessions.forEach(s => { if (s.sets.length > maxSets) maxSets = s.sets.length; });
  });

  // Build one series per set index
  const seriesData: { weekLabel: string; weights: (number | null)[] }[] = weeks.map((w, wIdx) => {
    const sessions = sessionsByWeek[wIdx] || [];
    // Use last session of the week if multiple
    ...
    return { weekLabel: w.label, weights };
  });

  return { seriesData, maxSets, weeks };
}
...
export default function GymTracker() {
  ...
  const getLogForDate = (dk: string) => {
    const entry = Object.entries(data.log || {}).find(([key]: any) =>
      key.startsWith(dk) && (data.log[key] as any)._complete
    );
    if (!entry) return null;
    const [key, val]: any = entry;
    const groupId = key.substring(11);
    if (groupId === "descanso") return { groupId: "descanso", groupName: "Descanso", exCount: 0, totalSets: 0 };
    ...
    <div style={{ fontFamily: "'DM Mono', 'Courier New', monospace", background: "#0d0d0d", minHeight: "100vh", color: "#e8e0d0" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tab-btn { background: transparent; border: none; color: #555; padding: 10px 14px; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; transition: all 0.15s; border-bottom: 2px solid transparent; white-space: nowrap; }
        ...
        .move-btn { background: transparent; border: 1px solid #222; color: #444; font-size: 12px; padding: 2px 7px; border-radius: 2px; transition: all 0.15s; }
        ...
        .dropdown-btn { width: 100%; background: #141414; border: 1px solid #2a2a2a; color: #e8e0d0; padding: 10px 14px; font-size: 13px; font-family: inherit; border-radius: 3px; display: flex; align-items: center; justify-content: space-between; transition: border-color 0.15s; text-align: left; }
        .dropdown-btn:hover, .dropdown-btn.open { border-color: #c8ff00; }
        .dropdown-menu { background: #141414; border: 1px solid #2a2a2a; border-radius: 3px; overflow: hidden; box-shadow: 0 8px 24px rgba(0,0,0,0.6); margin-top: 2px; }
        .dropdown-item { padding: 10px 14px; font-size: 13px; cursor: pointer; display: flex; align-items: center; gap: 10px; transition: background 0.1s; }
              ...
              <div key={g.id} className="group-card">
                ...
                {activeGroupId === g.id && (
                  <div style={{ padding: "0 13px 13px", borderTop: "1px solid #181818" }}>
                    ...
                    {g.id === "descanso" ? (
                      <div className="empty" style={{ padding: "10px 0", color: "#2a2a2a" }}>Sin ejercicios</div>
                    ) : (
                      <>
